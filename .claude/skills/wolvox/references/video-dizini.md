...
- Wolvox videolarının Türkçe altyazıları okundu; çıkarılan bilgiler `video-ozetleri.md` dosyasında (kaynak `{ID}`). Ekrandaki tıklamalar altyazıya girmediği için ayrıntı eksik olabilir. **Başlıklar** da menü yolunu içerdiği için değerli: `WolvoxCloud - Finans Yönetimi - Raporlar - ...` gibi başlıklar menü hiyerarşisini verir (`menu-haritasi.md`).
